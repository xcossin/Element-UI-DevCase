<template>
  <div class="">
    <div class="component-des-box">
      ...
      <p class="mar-tAb-20">
        2、通过组件回调方法可以获取到拥有raw属性的file对象，或者文件对象的raw属性，二者都可以转成二进制。
        <ul class="ul-li-tb-5">
          <li>但是 on-success、on-progress 方法，只有在 action 定义接口并调用后才能获取到方法的回调参数以及文件对象的raw属性，可以转成二进制。</li>
          <li>before-upload、on-change回调参数 file的raw值 可以通过 new Blob() 方法转成二进制文件流。</li>
          <li>on-change 无论 action 是否定义接口都能获取到回调参数</li>
        </ul>
      </p>
      <el-upload
        ref="upload"
    ...
    <!-- <el-collapse class="w800p f18" v-model="activeNames">
      <el-collapse-item title="如何获取二进制文件" name="1">
        <p>有些时候，后端接口需要上传二进制文件类型。</p>
        <p>
          两种方式获取<br>
          1、action参数直接定义好上传接口，直接使用el-upload组件上传文件，文件类型就是二进制流<br>
          2、通过on-change方法可以获取到文件对象的raw属性，可以转成二进制。<br>
          3、获取到文件后转成二进制。
        </p>
      </el-collapse-item>
    </el-collapse> 
...
export default {
  ...
  methods: {
    ...
    handleUploadChange (file, fileList) {
      ...
      console.log('this.changeFile', this.changeFile)
